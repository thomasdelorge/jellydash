...
final class HistoryEditServiceTest extends TestCase
{
    ...
    public function testPayloadFlagsWatchTimePastRuntimeAndListsSiblings(): void
    {
        $id = $this->insertPlay([
        ...
        $this->assertTrue($payload['exceeds_runtime']);
        $this->assertSame(86400, $payload['watched_sec']);
        $this->assertSame(3120, $payload['runtime_sec']);
        $this->assertSame(1, $payload['sibling_count']);
        $this->assertSame($siblingId, $payload['siblings'][0]['id']);
    }

    public function testUpdateWatchedClearsOverRuntimeFlag(): void
    {
        $id = $this->insertPlay([
            'session_key' => 'phpunit-edit-clamp',
            'watched_sec' => 86400,
            'runtime_sec' => 3600,
        ]);

        $payload = $this->service->updateWatched($id, 3600);

        $this->assertNotNull($payload);
        $this->assertSame(3600, $payload['watched_sec']);
        $this->assertFalse($payload['exceeds_runtime']);
