(function () {
    ...
    const warnEl = dialog.querySelector('[data-history-warn]');
    const warnCopy = dialog.querySelector('[data-history-warn-copy]');
    const clampBtn = dialog.querySelector('[data-history-clamp]');
    ...
    const runtimeEl = dialog.querySelector('[data-history-runtime]');
    ...
    if (
        !userEl || !titleEl || !subEl || !metaEl || !warnEl || !warnCopy || !clampBtn
        || !hoursInput || !minutesInput || !secondsInput || !runtimeEl
    ...
    function updateWarn() {
        if (!current || current.runtime_sec <= 0) {
            warnEl.hidden = true;

            return;
        }

        const watched = watchedFromInputs();
        const over = watched > current.runtime_sec;
        warnEl.hidden = !over;
        if (over) {
            warnCopy.textContent = 'Entered '
                + formatDuration(watched)
                + ', runtime is '
                + (current.runtime_label || formatDuration(current.runtime_sec))
                + '.';
        }
    }
    ...
    function fill(play) {
        ...
        runtimeEl.textContent = play.runtime_sec > 0
            ? 'Title runtime: ' + play.runtime_label
            : 'Title runtime is unknown.';
    ...
    clampBtn.addEventListener('click', function () {
        if (!current || current.runtime_sec <= 0) {
            return;
        }

        fillDuration(current.runtime_sec);
        updateWarn();
        runMutation({
            action: 'update',
            id: current.id,
            watched_sec: current.runtime_sec,
        });
    });
